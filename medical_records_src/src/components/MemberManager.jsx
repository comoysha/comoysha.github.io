...
export default function MemberManager({ open, onClose, members, onUpdate }) {
  const [draft, setDraft] = useState(members);
  const [newName, setNewName] = useState("");
  const [newEmoji, setNewEmoji] = useState("👤");
  const colors = ["#4A7C6F", "#C06B5D", "#7B6B8D", "#5B7FA5", "#A08C5B", "#5BA07C"];

  useEffect(() => { if (open) { setDraft(members); setNewName(""); setNewEmoji("👤"); } }, [open, members]);
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }} onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: "#fff", borderRadius: 14, padding: 16, width: "100%", maxWidth: 480, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 4px 24px rgba(0,0,0,0.15)" }}>
        <div style={{ fontSize: 15, fontWeight: 600, color: "#333", marginBottom: 12, fontFamily: "'Noto Sans SC', sans-serif" }}>管理成员</div>
        {draft.map((m) => (
          <div key={m.id} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
            ...
            <input value={m.name} onChange={function(e) { updateMemberField(m.id, "name", e.target.value); }} style={{
              flex: 1, minWidth: 0, padding: "6px 10px", borderRadius: 8, border: "1.5px solid #e8e5e0",
            ...
            {draft.length > 1 && (
              <button onClick={() => handleRemove(m.id)} style={{
                ...
                color: "#c06b5d", fontSize: 14, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0,
              }}>×</button>
            )}
          </div>
        ))}
        {draft.length < 8 && (
          <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
            ...
            <input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="新成员姓名" style={{
              flex: 1, minWidth: 0, padding: "8px 12px", borderRadius: 10, border: "1.5px solid #e8e5e0", fontSize: 13, fontFamily: "'Noto Sans SC', sans-serif", outline: "none",
            }} onKeyDown={(e) => e.key === "Enter" && handleAdd()} />
            <button onClick={handleAdd} style={{
              padding: "8px 14px", borderRadius: 10, border: "none", background: newName.trim() ? "#4A7C6F" : "#ddd",
              color: "#fff", fontSize: 13, cursor: newName.trim() ? "pointer" : "default", fontFamily: "'Noto Sans SC', sans-serif", flexShrink: 0,
            }}>添加</button>
          </div>
        )}
